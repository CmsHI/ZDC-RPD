void get_avg_signal_lowresEMP100(std::string infile="out_example"){
    ...
    auto fill_graphs=[&freq,&graph1,&graph2,&graph3,&graph4,&graph5](std::vector<double>& ch1,std::vector<double>& ch2,std::vector<double>& ch3,std::vector<double>& ch4,std::vector<double>& ch5){
        for (int i = 0; i < 5; i++)
        {
            ...
            graph2->SetBinContent(i+1,ch2[i]*1.11);
            graph3->SetBinContent(i+1,ch3[i]*1.29);
            graph4->SetBinContent(i+1,ch4[i]*1.68);
