void plot_weightedchargespos_pos_2d(std::string filename,int filter_num_of_peaks=1,double rel_gain1=1,double rel_gain2=1,double rel_gain4=1, double rel_gain5=1){
    ...
    auto hist2d=histo_df.Histo2D({"hist2d", "Charge weighted channel number vs position", 62,*(histo_df.Min<double>("position")), *(histo_df.Max<double>("position")), 100u, 0, 6},"position","charge_weighted_pos");

    TF1* lin=new TF1("lin","x/[1]+[0]",-11,53);
    lin->SetParameters(200,-15);
    TCanvas* c1=new TCanvas();
    hist2d->GetXaxis()->SetTitle("Position [mm]");
    hist2d->GetYaxis()->SetTitle("Channel number");
    hist2d->Fit("lin","R");
