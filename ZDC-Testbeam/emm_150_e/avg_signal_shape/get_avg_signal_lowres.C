void get_avg_signal_lowres(std::string infile="out_example"){
    ...
    auto fill_graphs=[&freq,&graph1,&graph2,&graph3,&graph4,&graph5](std::vector<double>& ch1,std::vector<double>& ch2,std::vector<double>& ch3,std::vector<double>& ch4,std::vector<double>& ch5){
        for (int i = 0; i < 5; i++)
        {
            ...
            graph1->SetBinContent(i+1,ch1[i]);
            graph2->SetBinContent(i+1,ch2[i]);
            graph3->SetBinContent(i+1,ch3[i]);
            graph4->SetBinContent(i+1,ch4[i]);
            graph5->SetBinContent(i+1,ch5[i]);
        }
