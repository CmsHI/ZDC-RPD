void plot_total_charge_upd1(std::string filename, int filter_num_of_peaks=1,double rel_gain1=1,double rel_gain2=1,double rel_gain4=1,double rel_gain5=1){
    ...
    auto energy_filter=[&rel_gain1,&rel_gain2,&rel_gain4,&rel_gain5](double& charge1,double& charge2,double& charge3,double& charge4,double& charge5){
        return !(rel_gain1*charge1<4000 && rel_gain2*charge2<4000 && charge3<4000 &&rel_gain4*charge4<4000 &&rel_gain5*charge5<4000);
    };
    ...
    auto new_df=charge_df.Define("total_charge",calc_tot_charge,{"charge1","charge2","charge3","charge4","charge5"})
                         ...
                         .Filter("position<50. && position>-13.");


    TCanvas* c1=new TCanvas();
    TF1 mygaus("mygaus","gaus(0)+gaus(3)");
    mygaus.SetParNames("height 1","mean 1","sigma 1","height 2","mean 2","sigma 2");
    mygaus.SetParameters(5000,4500,700,5000,0,600);
    auto tmp_hist=new_df.Histo1D({"Fit parameters","Total charge",300,-2000,12000},"total_charge");
