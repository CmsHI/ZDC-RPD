...
void plot_charge_pos_2d_conv(std::string filename,int filter_num_of_peaks=1){
    ...
    auto hist1=histo_df.Histo2D({"hist1", "Charge [fC] of channel 1 vs position", 62,*(histo_df.Min<double>("position")), *(histo_df.Max<double>("position")), 200u, *(histo_df.Min<double>("charge1"))-500, *(histo_df.Max<double>("charge1"))+500},"position","charge1");
    auto hist2=histo_df.Histo2D({"hist2", "Charge [fC] of channel 2 vs position", 62,*(histo_df.Min<double>("position")), *(histo_df.Max<double>("position")), 200u, *(histo_df.Min<double>("charge2"))-500, *(histo_df.Max<double>("charge2"))+500},"position","charge2");
    auto hist3=histo_df.Histo2D({"hist3", "Charge [fC] of channel 3 vs position", 62,*(histo_df.Min<double>("position")), *(histo_df.Max<double>("position")), 200u, *(histo_df.Min<double>("charge3"))-500, *(histo_df.Max<double>("charge3"))+500},"position","charge3");
    auto hist4=histo_df.Histo2D({"hist4", "Charge [fC] of channel 4 vs position", 62,*(histo_df.Min<double>("position")), *(histo_df.Max<double>("position")), 200u, *(histo_df.Min<double>("charge4"))-500, *(histo_df.Max<double>("charge4"))+500},"position","charge4");
    auto hist5=histo_df.Histo2D({"hist5", "Charge [fC] of channel 5 vs position", 62,*(histo_df.Min<double>("position")), *(histo_df.Max<double>("position")), 200u, *(histo_df.Min<double>("charge5"))-500, *(histo_df.Max<double>("charge5"))+3500},"position","charge5");

    //convolution of gauss with box distribution
    
    TF1* fitfunc=new TF1("fitfunc",convolutionfunc,-65,217,4);
    fitfunc->SetParLimits(0,0,30);
    fitfunc->SetParLimits(1,0,50000);
    fitfunc->SetParLimits(2,-65,90);
    fitfunc->SetParLimits(3,0,10);
    
    fitfunc->SetParNames("box width","gaus height","gaus mean","gaus sigma");
    fitfunc->SetParameters(16,400,-10,5);
