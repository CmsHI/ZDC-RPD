void make_pos_charge_root_update2emm(std::string ref_file="example"){
    ...
    TFile* output_file=new TFile((ref_file+"_charge_pos_upd2emm.root").c_str(),"RECREATE");
    ...
    for (int i = 0; i < filenames.size(); i++)
    {
        ...
        //ch2 is much noiser
        auto seconditer_bcg_ch2=[&window,&n_seciter_bcg,&sec_bcgint,&min_pos,&min_bcg,&was_peak,&peak_threshold](std::vector<int>& signal,double& firstiter_bcg){
            n_seciter_bcg=0;
            sec_bcgint=0;
            ...
            if(was_peak==0){
                for(int i=0;i<window;i++){
                    if(TMath::Abs((double)signal[i]-firstiter_bcg)<3){
                        sec_bcgint+=signal[i];
                        n_seciter_bcg++;
                    }
                }
                ...
                for(int i=0;i<window;i++){
                    if((TMath::Abs((double)signal[i]-firstiter_bcg)<3)&&((i<min_pos-6)||(i>min_pos+16))){
                        sec_bcgint+=signal[i];
                        n_seciter_bcg++;
                    }
                }
        ...
        auto curr_df= ch_df
                ...
                .Define("ch2_secbcg",seconditer_bcg_ch2, {"ch2","ch2_bcg"})
